...
def EV_F_ITER(X, k_init, theta_init, n_agents, grid_list):
    ...
    for i in range(len(grid_list)):
        exp += grid_list[i].evaluate(knext)  #####CHANGED 

    VT_sum=utility(cons, lab) + beta * (1./5) * exp  #####CHANGED 
...
def EV_GRAD_F(X, k_init, theta_init, n_agents):
    ...
    for ixN in range(N):
        ...
        if (xAdj[ixN] - h >= 0):
            xAdj[ixN]=X[ixN] + h            
            fx2=EV_F(xAdj, k_init, theta_init, n_agents)  #####CHANGED 
            
            xAdj[ixN]=X[ixN] - h
            fx1=EV_F(xAdj, k_init, theta_init, n_agents)  #####CHANGED 
...
def EV_GRAD_F_ITER(X, k_init, theta_init, n_agents, grid_list):
    ...
    for ixN in range(N):
        ...
        if (xAdj[ixN] - h >= 0):
            xAdj[ixN]=X[ixN] + h            
            fx2=EV_F_ITER(xAdj, k_init, theta_init, n_agents, grid_list)  #####CHANGED 
            
            xAdj[ixN]=X[ixN] - h
            fx1=EV_F_ITER(xAdj, k_init, theta_init, n_agents, grid_list)  #####CHANGED 
            
            GRAD[ixN]=(fx2-fx1)/(2.0*h)
            
        else:
            xAdj[ixN]=X[ixN] + h
            fx2=EV_F_ITER(xAdj, k_init, theta_init, n_agents, grid_listd)  #####CHANGED 
    ...
    if (flag):
        ...
        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G(xAdj, k_init, theta_init, n_agents)  #####CHANGED 
...
def EV_JAC_G_ITER(X, flag, k_init, theta_init, n_agents):
    ...
    if (flag):
        ...
        # Finite Differences
        h=1e-4
        gx1=EV_G_ITER(X, k_init, theta_init, n_agents)  #####CHANGED 
        
        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G_ITER(xAdj, k_init, theta_init, n_agents)  #####CHANGED 
                A[ixN + ixM*N]=(gx2[ixM] - gx1[ixM])/h
        return A    
